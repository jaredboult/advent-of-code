...
function solveProblem() {
  ...
  createFloor(features, ranges);
  let simulationComplete = false;
  while (!simulationComplete) {
    simulationComplete = placeNextSandUnit(features);
  }
...
function countSandUnits(features: Map<string, string>) {
  return Array.from(features.values()).filter((v) => v === "Sand").length;
}

function placeNextSandUnit(features: Map<string, string>) {
  ...
  while (!stoppedMoving) {
    ...
    if (sandLocation.equals(new Point(500, 0))) {
...
function printState(features: Map<string, string>) {
  ...
  // add extra 1 cell each side for visualisation
  minX -= 1;
  minY -= 1;
  maxX += 1;
  maxY += 1;
...
function createFloor(features: Map<string, string>, ranges: number[]) {
  let [minX, maxX, _minY, maxY] = ranges;

  const floorLevel = maxY + 2;
  minX -= 200;
  maxX += 200;

  const start = new Point(minX, floorLevel);
  const end = new Point(maxX, floorLevel);
  getAllWallPoints(start, end).forEach((point) => {
