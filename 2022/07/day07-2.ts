...
const tree = constructFilesystemTree(inputs);
console.log(tree);
const diskSpace = 70000000;
const totalSpaceUsed = tree?.getTotalSize() ?? 0;

console.log("Total Space Used: ", totalSpaceUsed);

const freeSpace = diskSpace - totalSpaceUsed;
console.log("Free space: ", freeSpace);

const requiredFreeSpace = 30000000;
const amountToDelete = requiredFreeSpace - freeSpace;
console.log("Amount to delete: ", amountToDelete);

const result = directories
  .map((d) => d.getTotalSize())
  .filter((s) => s >= amountToDelete)
  .sort((a, b) => a - b)
  .shift();
console.log(result);
